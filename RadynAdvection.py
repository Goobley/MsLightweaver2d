...
@njit(cache=True)
def anj_direct(z0, z1, vz0, vz1, an0, an1, dt):
    # i1 = i0 + 1
    # z0 = z[0]
    # z1 = z[1]
    # vz0 = vz[0]
...
@njit(cache=True)
def flux_calc_direct(z0, z1, d0, d1, vz0, vz1, dtn):
    i1 = i0 + 1
    # z0 = atmost.z1[i0]
    # z1 = atmost.z1[i1]
    # d0 = atmost.d1[i0]
...
def an_sol_my_grid_no_vel(atmost, z0, z1, d0, d1, n0, dtn, maxIter=100, tol=1e-5, lineSearch=True):
    vz0 = np.zeros_like(z0)
    vz1 = np.zeros_like(z1)
    theta = 0.55

    fmj = flux_calc_direct(z0, z1, d0, d1, vz0, vz1, dtn)
    an0 = n0 / d0
    ...
    def objective(n1):
        ...
        # anj = anj_pro(atmost.z1[i0:i0+2], atmost.vz1[i0:i0+2], atmost.dt[i1], i0, an0, an1)
        anj = anj_direct(z0, z1, vz0, vz1, an0, an1, dtn)
